...
const EBMeterCalculation = () => {
  ...
  const fetchMeterNumbers = async () => {
    try {
      const response = await meterNumberService.getActiveMeterNumbers(
        currentBranchId
      );
      if (response.success) {
  ...
  const openMeterModal = (meter = null) => {
    ...
    if (meter) {
      ...
      const formattedReadingDate = new Date(meter.readingDate)
        .toISOString()
        .split("T")[0];
      const formData = {
        ...initialMeterForm,
      ...
      const tomorrowReading = meterReadings.find(
        (r) => r.readingDate === tomorrow
      );
  ...
  const saveMeterNumber = async () => {
    try {
      ...
      const response = await meterNumberService.createMeterNumber(
        meterNumberForm
      );
      if (response.success) {
    ...
    setMeterForm((prev) => {
      ...
      if (name === "startKWH") {
        ...
        if (end === 0 && updated.readingDate) {
          ...
          const nextDayReading = meterReadings.find(
            (r) => r.readingDate === nextDayStr
          );
  ...
  const checkAutoFillAvailability = () => {
    ...
    if (nextDayReading && nextDayReading.startKWH > 0) {
      return {
        available: true,
        message: `Auto-fill available: Next day's start KWH (${formatKWH(
          nextDayReading.startKWH
        )}) can be used for ${new Date(
          meterForm.readingDate
        ).toLocaleDateString()}'s end KWH.`,
    ...
    return {
      available: false,
      message: `No next day data available for auto-fill from ${new Date(
        meterForm.readingDate
      ).toLocaleDateString()}.`,
      nextDayKWH: 0,
    };
  ...
  const saveMeter = async (e) => {
    ...
    if (!meterForm.endKWH || meterForm.endKWH === 0) {
      if (
        !window.confirm(
          "End KWH is empty. This will set consumption to 0. Do you want to continue?"
        )
      ) {
        return;
      }
    ...
    try {
      ...
      if (editingMeter) {
        ...
        setMeterReadings((prev) =>
          prev.map((meter) =>
            meter._id === editingMeter._id
              ? { ...meterForm, _id: meter._id }
              : meter
          )
        );
      } else {
  ...
  const deleteMeter = async (meterId) => {
    if (window.confirm("Are you sure you want to delete this meter reading?")) {
      try {
        ...
        // Simulate API call - replace with actual service
        setMeterReadings((prev) =>
          prev.filter((meter) => meter._id !== meterId)
        );
      } catch (error) {
  ...
  const totalUnits = meterReadings.reduce(
    (sum, meter) => sum + meter.consumption,
    0
  );
  const averageConsumption =
    meterReadings.length > 0 ? totalUnits / meterReadings.length : 0;
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: (value) => (
        <span
          className={`font-semibold ${
            value && value > 0 ? "text-orange-600" : "text-gray-400"
          }`}
        >
    ...
    },
    {
      ...
      renderCell: (value) => (
        <span
          className={`font-semibold ${
            value && value > 0 ? "text-green-600" : "text-gray-400"
          }`}
        >
          {formatKWH(value)}
        </span>
  ...
  const filteredMeterReadings = meterReadings.filter((meter) => {
    ...
    const meterNumberMatch =
      !meterNumberFilter || meter.meterNumber === meterNumberFilter;
    const searchMatch =
      meter.meterNumber?.toLowerCase().includes(q) ||
      meter.billNumber?.toLowerCase().includes(q);
    const dateMatch =
      !dateRange.startDate ||
      new Date(meter.readingDate) >= new Date(dateRange.startDate);
    const endDateMatch =
      !dateRange.endDate ||
      new Date(meter.readingDate) <= new Date(dateRange.endDate);
      ...
      <div className="px-4 py-6 sm:px-6">
        ...
        <div className="bg-card rounded-lg p-6 shadow-sm border border-border mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="text-center p-3 bg-muted rounded-lg">
              <div className="text-2xl font-bold text-foreground">
                {(totalUnits / Math.max(meterReadings.length, 1)).toFixed(2)}
              </div>
        ...
        <div className="hidden lg:block bg-card rounded-2xl shadow-lg border border-border overflow-hidden">
          <div className="px-6 py-4 border-b border-border bg-muted">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Total: {filteredMeterReadings.length} records
            </p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-3">
                    <div className="flex items-center">
                      ...
                      <span className="text-foreground font-medium">
                        {new Date(meter.readingDate).toLocaleDateString()}
                      </span>
                    </div>
                    <div className="flex items-center">
                      <span className="w-24 text-sm font-medium text-muted-foreground">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center">
                      ...
                      <span className="text-foreground font-medium">
                        {formatKWHWithFallback(meter.endKWH)}
                      </span>
                    </div>
                    <div className="flex items-center">
                      ...
                      <span className="ml-1 font-medium text-primary">
                        {formatKWH(meter.consumption)} KWH
                      </span>
                    </div>
                  </div>
                </div>
            ...
            {filteredMeterReadings.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Get started by creating a new meter reading.
                </p>
              </div>
            ) : (
              <div className="space-y-3">
                {filteredMeterReadings.map((meter) => (
                  ...
                  >
                    <div
                      ...
                      onClick={() =>
                        setExpandedMeter(
                          expandedMeter === meter._id ? null : meter._id
                        )
                      }
                    >
                      <div className="flex justify-between items-center">
                        <div className="flex-1">
                          ...
                          <div className="text-xs text-muted-foreground">
                            PF: {meter.pf} • {formatKWH(meter.startKWH)} →{" "}
                            {formatKWHWithFallback(meter.endKWH)} •{" "}
                            {formatKWH(meter.consumption)} KWH
                          </div>
                        </div>
      ...
      >
        ...
        >
          ...
          {(() => {
            ...
            const hasAutoFilled =
              meterForm.endKWH > 0 && autoFillInfo.available;
              ...
              <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
                <div className="flex items-center">
                  ...
                  <span className="text-green-700 text-sm">
                    ✓ Auto-filled End KWH from next day's start reading (
                    {autoFillInfo.nextDayKWH.toFixed(2)})
                  </span>
                </div>
              </div>
            ) : null;
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <FormInput
              ...
              type="date"
              value={
                meterNumberForm.installationDate ||
          ...
          <div className="flex justify-end space-x-3 pt-6 border-t border-border">
            <UIButton
              type="button"
              onClick={closeMeterNumberModal}
              variant="secondary"
            >
              Cancel
            </UIButton>
            <UIButton type="submit">Create Meter Number</UIButton>
          </div>
        </form>
      </DialogBox>
    </div>
