...
const VendorManagement = () => {
  ...
  const handleVendorSubmit = async (e) => {
    ...
    try {
      let response;
      if (editingVendor) {
        response = await vendorService.updateVendor(editingVendor._id, vendorForm, selectedFiles);
      } else {
  ...
  const filteredVendors = vendors.filter(vendor => {
    if (searchFilter && !vendor.vendorName.toLowerCase().includes(searchFilter.toLowerCase()) &&
    ...
    if (vendorTypeFilter && vendor.vendorType !== vendorTypeFilter) return false;
    if (statusFilter && vendor.status !== statusFilter) return false;
    if (balanceStatusFilter) {
      if (balanceStatusFilter === 'settled' && vendor.currentBalance !== 0) return false;
      if (balanceStatusFilter === 'vendor_owes_us' && vendor.currentBalance <= 0) return false;
      if (balanceStatusFilter === 'we_owe_vendor' && vendor.currentBalance >= 0) return false;
    }
    return true;
  });

  const vendorColumns = [
    "Code", "Name", "Contact", "Phone", "GSTIN", "PAN", "Place of Supply", 
    "Type", "Status", "Balance", "Actions"
  ];
  ...
  const transformVendorDataForTable = (vendors) => {
    return vendors.map(vendor => ({
      ...
      Type: vendor.vendorType?.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase()) || '-',
        ...
        <form onSubmit={handleVendorSubmit} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <FormInput
              ...
              onChange={(e) => setVendorForm({...vendorForm, placeOfSupply: e.target.value})}
              required
            />
            <FormInput
              label="GST Number"
            ...
            <FormInput
              ...
              onChange={(e) => setVendorForm({...vendorForm, creditLimit: parseFloat(e.target.value) || 0})}
            />
            <FormSelect
              label="Payment Terms"
        ...
        <form onSubmit={handleTransactionSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <FormSelect
              ...
              options={[
                { value: 'payment_given', label: 'Payment Given (We paid vendor)' },
                { value: 'payment_received', label: 'Payment Received (Vendor paid us)' },
          ...
          {transactionForm.type === 'adjustment' && (
            <FormSelect
              ...
              onChange={(e) => setTransactionForm({...transactionForm, adjustmentType: e.target.value})}
              options={[
