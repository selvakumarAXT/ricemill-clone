...
const PaddyManagement = () => {
  ...
  const calculateSummaryStats = () => {
    const stats = {
      ...
      totalExpectedRice: 0,
      totalExpectedByproducts: 0,
      totalActualRice: 0,
      totalActualByproducts: 0,
      totalRiceShortage: 0,
      totalRiceSurplus: 0,
      totalByproductsShortage: 0,
      totalByproductsSurplus: 0,
    };
    ...
    if (paddies.length > 0) {
      ...
      paddies.forEach((paddy) => {
        ...
        // Distribution calculations (67% rice, 33% byproducts)
        const paddyWeight = paddy.paddy?.weight || 0;
        const expectedRice = paddyWeight * 0.67;
        const expectedByproducts = paddyWeight * 0.33;
        const actualRice = paddy.distribution?.actual?.riceKg || 0;
        const actualByproducts = paddy.distribution?.actual?.byproductsKg || 0;

        stats.totalExpectedRice += expectedRice;
        stats.totalExpectedByproducts += expectedByproducts;
        stats.totalActualRice += actualRice;
        stats.totalActualByproducts += actualByproducts;

        // Calculate shortages and surpluses
        const riceDiff = expectedRice - actualRice;
        const byproductsDiff = expectedByproducts - actualByproducts;

        if (riceDiff > 0) {
          stats.totalRiceShortage += riceDiff;
        } else {
          stats.totalRiceSurplus += Math.abs(riceDiff);
        }

        if (byproductsDiff > 0) {
          stats.totalByproductsShortage += byproductsDiff;
        } else {
          stats.totalByproductsSurplus += Math.abs(byproductsDiff);
        }
              ...
              <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                ...
                <div className="bg-gray-50 rounded-lg p-3">
                  ...
                  <div className="text-gray-600">
                    ...
                    <div>
                      • Total Rice: {formatWeight(summaryStats.totalRiceOutput)}
                    </div>
                  </div>
                </div>
